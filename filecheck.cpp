		...
		while (std::getline(file, str))		// Loops File lines
		{
			...
			if (str.substr(0, 1) != "T")	// T Check
			{
				...
				case CITY_FILE:				// City File Check
					citylabel = stringToUpper(str.substr(0, 20));
					city = stringToUpper(str.substr(20, 20));
					state = stringToUpper(str.substr(40, 2));
				...
				case ROUTE_FILE:			// Route File Check
					...
					if (actionCode == "A")
					{
						// First, check to see if route number already exists...
						for (auto it : mRouteCityMap)
						{
							if (it.second == routeNum)
							{
								logError(fileName, dateString, seqNum, lineNumber, "Route number already exists, Skipping line.");
								lineValid = false;
								break;
							}
						}
						...
						if (lineValid)
						{
							vector<string> routeCities;
							int pos = 5;
							citylabel = stringToUpper(str.substr(pos, 20));  // first city label in route
							...
							routeCities.push_back(citylabel);

							for (pos = 25; pos <= 185 && lineValid; pos += 20)		// Loops CityLabel 2-9 if they exist.
							{
								citylabel = stringToUpper(str.substr(pos, 20));
								if (citylabel == "                    ")
									break;
							...
							// Now add checked valid route to mRouteCityMap
							if (lineValid)
							{
								for (auto it : routeCities)
									mRouteCityMap[it] = routeNum;
							}
						}
					}
					...
					{
						// First, check to see if route number to be changed exists...
						lineValid = false;
						for (auto it : mRouteCityMap)
						{
							if (it.second == routeNum)
						...
						if (lineValid == false)
							logError(fileName, dateString, seqNum, lineNumber, "Route in Change Action does not exist, skipping line");
						else
						{
							citylabel = stringToUpper(str.substr(5, 20));
					...
					else if (actionCode == "D")		// Delete Action
					{
						...
						if (deletions.size() == 0)
							logError(fileName, dateString, seqNum, lineNumber, "Route in delete action does not exist, Skipping line.");
...
// Utility function to convert a std::string to all caps for case-insensitive comparison
string fileCheck::stringToUpper(string str)
{
